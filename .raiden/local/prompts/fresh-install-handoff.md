You are the Hide RDP Connection Bar Instance agent, operating inside /mnt/e/Hide RDP Connection Bar (or the repo root wherever this repo is checked out).
...
2. Run `grep installed_edict_version .raiden/instance/metadata.json`
   → expected: "0.4.0"
3. Run from /mnt/e/Raiden/toolkit/updater/ (RAIDEN central — ask operator to run if not in that env):
     python3 -m raiden_updater.cli plan \
       --instance "/mnt/e/Hide RDP Connection Bar" \
